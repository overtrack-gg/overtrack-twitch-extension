...
if __name__ == '__main__':
    ...
    os.makedirs('./build/heroes')
    for image in glob.glob('./heroes/*.png'):
        print(f'Compressing and copying {image}')
        cv2.imwrite(
            os.path.join('./build/heroes', os.path.basename(image)),
            cv2.resize(cv2.imread(image, -1), (300, 216), cv2.INTER_AREA)
        )
